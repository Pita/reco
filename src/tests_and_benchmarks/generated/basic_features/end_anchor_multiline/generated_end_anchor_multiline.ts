...
const atom0000 = (i: number, str: string, context: Context): number => {
  if (i !== str.length) {
    const charCodeMultiLineEndAnchor0 = str.charCodeAt(i);
    let isNewLineChar0 = false;
    // TODO: find way to generate these trees
    if (charCodeMultiLineEndAnchor0 <= 13) {
      isNewLineChar0 =
        charCodeMultiLineEndAnchor0 === 10 ||
        charCodeMultiLineEndAnchor0 === 13;
    } else {
      if (charCodeMultiLineEndAnchor0 <= 8233) {
        isNewLineChar0 = charCodeMultiLineEndAnchor0 >= 8232;
      }
    }
    if (!isNewLineChar0) {
      return -1;
